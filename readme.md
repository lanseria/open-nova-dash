...
* **参数名有 `par` 和 `str` 两种**：不同厂商固件对同一命令可能只认其中一种。例如录像控制 `2001`，GitUp Git2 用 `par=1/0`，而 BlackSys 等行车记录仪固件用 `str=1/0`。参数名不对时返回 `-22`（EINVAL）。
...
* **行车记录仪固件常禁用 APP 端录像开关**：`2001` 无论开始/停止都可能返回 `-22`，属固件安全设计（循环录像由设备自管），并非脚本错误。
* **错误码即 Linux errno**：`-22`=EINVAL（参数无效/状态不允许）、`-13`=抓拍执行失败、`-3`=存储或状态错误、`-1`=不支持。
...
* **获取文件缩略图**: `http://192.168.1.254/A:/Movie/20260831_1000.MP4?custom=1&cmd=4001`
