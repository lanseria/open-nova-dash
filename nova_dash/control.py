...
def capture(client) -> None:
    ...
    print("  → 直接拍照失败, 走模式切换连招: 照片模式 → 拍照 → 视频模式 → 恢复录像")
    client.send_cmd(3001, par=1, description="切换到照片模式 (par=1)", timeout=12)
    time.sleep(2)
    ...
    client.send_cmd(3001, par=0, description="切回视频模式 (par=0)", timeout=12)
    time.sleep(3)
...
def format_sd(client) -> None:
    ...
    if answer != "yes":
        print("已取消, 未做任何改动.")
        return
    root = client.send_cmd(3010, str_par=1, description="格式化 SD 卡", timeout=30)
    if is_ok(root):
