...
- **控制首页（第一 tab）**：进入自动开始 RTSP 直播（子码流低延迟，可切主码流）；录像状态跟随心跳实时展示，开/停按钮按状态自动启用；远程拍照（实测照片模式直拍连招）；相册入口仅在停止录像后开放（设备录像中拒绝文件列表）
- **状态页（第二 tab）**：固件版本、电池、SD 卡状态、剩余空间、心跳时间
- **相册（控制页推入）**：按天分组的封面卡片，视频封面优先用设备原生缩略图（下载 URL 追加 `?custom=1&cmd=4001`，实测直接返回 JPEG），失败回退 FFmpeg 流式抽帧（含磁盘缓存），点击即在线播放 TS 视频（KSPlayer/FFmpeg 拉流，无需先下载），支持下载导出到系统相册
...
- `try` 指令扫描台：逐条试射候选命令码（交互单发 / `--all` 自动连发 / `--cmd` 任意单发 / `--capture-lab` 拍照组合实验），以设备提示音与屏幕反应为判据，找出固件真正认的命令（2026-09-18 实测定稿：`2001` 只认 `par=` 写法）
- `thumb` 原生视频封面探测：对最新一段录像逐一试 `?4001`/`?4002` URL 变体、CGI `4001/4002`（含回放模式轮）、`.THM` 对照，命中自动存图到 `./thumbs/`
...
│   └── thumb.py               # 原生视频封面探测（4001/4002/?4001/.THM）
...
└── ios/
    ...
    └── NovaDash/
        ...
        └── Views/
            ├── DashboardView.swift    # 状态页仪表盘（第二 tab）
            ├── AlbumView.swift        # 相册（由控制页推入）+ 照片查看器 + 视频播放
            └── ControlView.swift      # 控制首页（第一 tab）：自动直播/录像/拍照 + 相册入口
```
